...
export default class Standing extends Component {
    ...
    findByCountryAndLeagueAndTeam = (event) => {
        ...
        axios.get("http://localhost:8082/api/standing",{ params: { countryName: this.state.country,leagueName:this.state.league,teamName:this.state.team }},axiosConfig)
            .then(response => {
    ...
    render() {
        const { country, league, team} = this.state;
